...
```bash
PATH="/Users/itoshion/.nodebrew/current/bin:$PATH" npm version VERSION --no-git-tag-version
```
Then update `"version"` in `manifest.json` to match.

Build and verify:
```bash
PATH="/Users/itoshion/.nodebrew/current/bin:$PATH" npm run build
```
...
Create the ZIP:
```bash
PATH="/Users/itoshion/.nodebrew/current/bin:$PATH" bash -c '
...
```bash
git tag -d vVERSION 2>/dev/null || true
git push origin --delete vVERSION 2>/dev/null || true
git tag vVERSION
